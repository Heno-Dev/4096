...
class playGame extends Phaser.Scene {
  ...
  create() {
    ...
    for(var i = 0; i < gameOptions.boardSize.rows; i++) {
      ...
      for(var j = 0; j < gameOptions.boardSize.cols; j++) {
        ...
        var tile = this.add.sprite(tilePosition.x, tilePosition.y,
          "tiles", 0);
        tile.visible = false;
  ...
  getTilePosition(row, col){
    var posX = gameOptions.tileSpacing * (col + 1) +
    gameOptions.tileSize * (col + 0.5);
    var posY = gameOptions.tileSpacing * (row + 1) +
  ...
  handleSwipe(e){
    if(this.canMove){
      ...
      var fastEnough = swipeTime < gameOptions.swipeMinDistance;
